...
async function getToken() {
  ...
  const res = await fetch(`${BASE_URL}/oauth/token`, {
    ...
    body: JSON.stringify({ client_id: CLIENT_ID, client_secret: CLIENT_SECRET, user_token: USER_TOKEN, grant_type: 'client_credentials' }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rmRes = await fetch(`${BASE_URL}/subscribers`, {
      ...
      body: JSON.stringify({ email, list_id: LIST_ID, first_name, last_name }),
    });
    const rmData = await rmRes.json();
    return res.status(200).json({ ok: true, data: rmData });
  } catch (err) {
